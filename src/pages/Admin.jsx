...
function Admin() {
  ...
  async function submitAdjustment(event) {
    ...
    try {
      ...
      setUsers((current) => current.map((user) => user._id === response.data.data.user._id ? { ...user, ...response.data.data.user } : user))
      ...
      <section className="card mt-5">
        ...
        <form className="mt-4" onSubmit={submitAdjustment}>
          ...
          <select className="field" id="admin-employee" value={selectedUserId} onChange={(event) => setSelectedUserId(event.target.value)} required>
            ...
            {users.filter((user) => user.role === 'employee').map((user) => <option key={user._id} value={user._id}>{user.name} — {user.department}</option>)}
          </select>
      ...
      <section className="card mt-5">
        ...
        <div className="table-wrapper">
          <table className="simple-table">
            <thead><tr><th>Name</th><th>Email</th><th>Department</th><th>Earned</th><th>Giving</th></tr></thead>
            <tbody>{users.filter((user) => user.role === 'employee').map((user) => (
              <tr key={user._id}><td>{user.name}</td><td>{user.email}</td><td>{user.department}</td><td>{user.earnedPoints}</td><td>{user.givingAllowance}</td></tr>
            ))}</tbody>
          </table>
        </div>
      </section>
